...
const ProductDetail = () => {
  ...
  return (
    <div className="flex flex-col overflow-hidden bg-white w-full">
      ...
      {/* SECTION 1: HERO - Exact Positioning based on Preply India */}
      <section 
        className="relative h-[75vh] md:h-[96vh] flex items-end pb-0 bg-no-repeat transition-all duration-1000 bg-cover bg-center lg:bg-[position:80%_center]" 
        style={{ 
          backgroundImage: `linear-gradient(to top, rgba(0,0,0,0.85) 0%, rgba(0,0,0,0.3) 50%, rgba(0,0,0,0.2) 100%), url(${product.heroImage || product.image})`,
        }}
      >
        <div className="container mx-auto px-6 lg:px-12 pb-20"> 
          <div className="max-w-5xl">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
              <h1 className="text-4xl md:text-5xl font-bold text-white leading-tight font-display mb-4">
                Introducing <span className="text-[#A3E635]">{product.name}</span>
              </h1>
              <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.4 }} className="text-lg md:text-xl text-white/90 font-light max-w-2xl">
                Engineered from recycled plastic, built for <span className="font-medium text-white">modern and sustainable living.</span>
              </motion.p>
            </motion.div>
          </div>
        </div>
      ...
      {/* SECTION 2: PRODUCT STORY */}
      <section className="py-20 lg:py-32 bg-[#DCEDC8]/40 border-y border-green-100"> 
        <div className="container mx-auto px-6 lg:px-12">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="text-3xl md:text-5xl font-display font-bold text-slate-900 mb-4">
              From Discarded Plastic to Designer Furniture
          ...
          <div className="relative">
            <div className="hidden lg:block absolute top-10 left-[12%] right-[12%] h-[2px] bg-[#2E7D32]/20 z-0" />
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-12 lg:gap-8 relative z-10">
              {steps.map((step, i) => (
                <div key={step.label} className="flex flex-col items-center text-center group">
                  <div className="w-20 h-20 rounded-[2rem] bg-white flex items-center justify-center mb-6 group-hover:bg-[#388E3C] transition-all duration-500 shadow-xl border border-green-200">
                    <step.icon className="h-8 w-8 text-[#2E7D32] group-hover:text-white" />
                  </div>
      ...
      {/* SECTION 3: ABOUT - Full Image (No Cutting) for Mobile & Desktop */}
        ...
        <div className="max-w-[1400px] mx-auto px-6">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-10 lg:gap-16 items-center">
            ...
            <div className="order-2 lg:order-1 lg:col-span-5">
              <div className="flex items-center gap-3 mb-6">
                <div className="w-10 h-[2px] bg-[#A3E635]"></div>
                <span className="text-slate-500 font-bold uppercase tracking-[0.2em] text-xs">Sustainable Engineering</span>
              </div>
              <h2 className="text-3xl md:text-5xl font-display font-bold mb-6 text-slate-900 leading-[1.1]">
                Redefining the <br className="hidden md:block"/> Future with {product.name}
              </h2>
              <p className="text-slate-600 leading-relaxed text-lg mb-10 border-l-4 border-[#A3E635]/30 pl-6">
                {product.fullDesc}
              </p>
              <Button asChild className="w-full sm:w-auto bg-[#166534] hover:bg-[#14532D] text-white font-bold px-10 py-7 rounded-full shadow-2xl">
                ...
                <a href={`https://wa.me/919558597555`} target="_blank"><MessageCircle size={20} />WhatsApp Enquiry</a>
              </Button>
            </div>

            {/* Images Right - Responsive Height and Container Fixed */}
            <div className="order-1 lg:order-2 lg:col-span-7 relative h-[300px] sm:h-[450px] md:h-[550px] lg:h-[600px] w-full">
              <div className="w-full h-full overflow-hidden rounded-[1.5rem] lg:rounded-[3rem] shadow-xl border border-slate-50">
                <AnimatePresence mode="wait">
                  <motion.img 
                    key={`bg-${currentIndex}`} 
                    src={sliderImages[currentIndex]} 
                    ...
                    animate={{ opacity: 1 }} 
                    transition={{ duration: 1 }} 
                    className="w-full h-full object-contain bg-slate-50 md:object-cover" 
                  />
                </AnimatePresence>
              </div>
              {/* Floating Image Hidden on Mobile to avoid clutter */}
              <div className="absolute bottom-[-10px] right-0 translate-x-[10%] w-[55%] h-[50%] overflow-hidden rounded-[2.5rem] shadow-2xl border-[6px] border-white z-10 hidden lg:block">
                <AnimatePresence mode="wait">
                  <motion.img key={`fg-${currentIndex}`} src={sliderImages[(currentIndex + 1) % 4]} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.6 }} className="w-full h-full object-cover" />
                </AnimatePresence>
              </div>
            </div>
          </div>
      ...
      {/* SECTION 4: FEATURES - Clear & High Contrast */}
      <section 
        className="relative py-24 lg:py-32 overflow-hidden bg-slate-950 bg-cover bg-center md:bg-fixed"
        style={{ backgroundImage: `linear-gradient(to bottom, rgba(10,26,18,0.92) 0%, rgba(10,26,18,0.6) 50%, rgba(10,26,18,0.92) 100%), url(${chinabbench1})` }}
      >
        <div className="container mx-auto px-6 lg:px-12 relative z-10">
          <div className="text-center mb-16">
            <h2 className="text-4xl md:text-6xl font-display font-extrabold text-[#A3E635] mb-6 tracking-tight ">{product.name}</h2>
            <h3 className="text-2xl md:text-5xl font-bold text-white opacity-95">Built Different. Built Better.</h3>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 lg:gap-10">
            {features.map((f) => (
              <div key={f.title} className="p-8 md:p-10 rounded-[2.5rem] bg-[#0d2119]/70 border border-white/10 backdrop-blur-sm transition-all group hover:bg-[#0d2119] hover:border-[#A3E635]/50 shadow-2xl">
                <div className="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-[#A3E635]/10 flex items-center justify-center mb-8 group-hover:bg-[#A3E635] transition-all">
                  <f.icon className="h-6 w-6 md:h-7 md:w-7 text-[#A3E635] group-hover:text-black" />
                </div>
                <h3 className="text-xl md:text-2xl font-bold text-white mb-4">{f.title}</h3>
                <p className="text-slate-300 text-base md:text-lg leading-relaxed">{f.desc}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* SECTION 5: COMPARISON - Mobile Scroll Enabled */}
      <section className="py-24 lg:py-32 bg-white">
        <div className="container mx-auto px-6 lg:px-12 max-w-full lg:max-w-fit overflow-hidden"> 
          <h2 className="text-3xl md:text-5xl font-display font-bold text-center mb-16">Reinventing Furniture Beyond Wood</h2>
          <div className="overflow-x-auto pb-4">
            <div className="min-w-[750px] mb-4 rounded-3xl border border-slate-200 shadow-xl overflow-hidden">
              <table className="w-full text-base border-collapse">
                <thead>
                  <tr className="bg-[#0F172A] text-white">
                    <th className="text-left p-6 font-semibold">Feature</th>
                    <th className="text-center p-6 font-semibold bg-[#1e293b]">{product.name}</th>
                ...
                <tbody className="divide-y divide-slate-100">
                  {rows.map((row) => (
                    <tr key={row.feature} className="hover:bg-slate-50 transition-colors">
                      <td className="p-6 font-medium text-slate-700 whitespace-nowrap">{row.feature}</td>
                      <td className="p-6 text-center font-bold text-[#2E7D32] bg-green-50/30">{renderValue(row.preply)}</td>
                      <td className="p-6 text-center text-slate-500">{renderValue(row.wood)}</td>
                    </tr>
                  ))}
                </tbody>
              </table>
      ...
      {/* SECTION 6: USE CASES */}
      <section className="py-24 lg:py-32 bg-[#F1F8F5]">
        <div className="container mx-auto px-6 lg:px-12">
          <div className="text-center max-w-3xl mx-auto mb-16">
            <h2 className="text-3xl md:text-5xl font-display font-bold text-slate-900">One {product.name}. Infinite Possibilities.</h2>
          </div>
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {cases.map((c) => (
              <div key={c.title} className="text-center p-8 rounded-3xl bg-white border border-slate-100 hover:border-[#2E7D32]/30 hover:shadow-xl transition-all group">
                <div className="w-16 h-16 mx-auto rounded-2xl bg-green-50 flex items-center justify-center mb-6 group-hover:bg-[#2E7D32] transition-colors">
                  <c.icon className="h-7 w-7 text-[#2E7D32] group-hover:text-white" />
                </div>
                <h3 className="text-lg font-bold text-slate-900 mb-2">{c.title}</h3>
                <p className="text-sm text-slate-600 leading-relaxed">{c.desc}</p>
      ...
      {/* <section className="py-24 lg:py-32 bg-[#0F172A] text-white">
        <div className="container mx-auto px-6 text-center">
          ...
          <div className="flex flex-col sm:flex-row justify-center gap-4">
            <Button variant="outline" className="h-14 px-10 rounded-full border-white/20 text-white bg-transparent hover:bg-white/10 text-lg font-bold">
