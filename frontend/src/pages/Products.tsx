...
import { MessageCircle } from "lucide-react";
import { Link } from "react-router-dom"; // 1. Link import zaroori hai
import plasticSheets from "@/assets/plastic-sheets.jpg";
import furniture from "@/assets/products-furniture.jpg";
import wasteCollection from "@/assets/waste-collection.jpg";
import chinabsofahero from "@/assets/chinabsofa-hero.png";
...
const products = [
  { id: "bin", name: "Wrapper Bin", desc: "Made from chocolate wrapper plastic waste...", image: wrapperhome, cta: "Adopt a Wrapper Bin" },
      ...
      <section className="py-24 md:py-32">
        <div className="container">
          <div className="max-w-2xl mb-16">
            ...
            <p className="text-muted-foreground text-lg">Every product carries an impact story.</p>
          </div>

          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            {products.map((p) => (
              <div key={p.name} className="relative group">
                {/* 2. CARD KO CLICKABLE BANANE KE LIYE LINK USE KIYA */}
                <Link 
                  to={`/product/${p.id}`} 
                ...
                >
                  ...
                  <div className="p-6">
                    ...
                    <a
                      href={`https://wa.me/919558597555?text=${encodeURIComponent(`Hello, I am interested in ${p.name}.`)}`}
                      target="_blank"
                      rel="noopener noreferrer"
